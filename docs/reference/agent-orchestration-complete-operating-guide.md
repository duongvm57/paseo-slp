# Agent Orchestration — Complete Operating Guide

> Supervisor–Lead–Peer (SLP), prerequisites, role/authority model, Paseo/control-plane wiring, `WORKSPACE_PROTOCOL.md`, attention/event/heartbeat model, multi-lane design, proof/acceptance, TDD governance, anti-patterns và continuous optimization.

### Cách agent phải đọc tài liệu này

- Đây là **operating guide**, không phải một prompt task cụ thể và không tự cấp thêm authority.
- Các invariant về authority, ownership, stable candidate, evidence chain và one-control-plane có ưu tiên cao hơn ví dụ minh họa.
- Các con số như heartbeat khoảng 15 phút chỉ là **ví dụ vận hành**, không phải hằng số bắt buộc; repo/protocol có thể chọn cadence khác.
- Khi một nhận định đến từ buổi trao đổi/kinh nghiệm thực hành, hãy coi đó là **operational heuristic**, không phải fact đã được kiểm chứng độc lập.
- Khi task thực tế mâu thuẫn với user authority hoặc `WORKSPACE_PROTOCOL.md`, không dùng guide này để tự mở rộng quyền.

---

## Executive summary

Nếu chỉ nhớ các invariant cốt lõi, hãy nhớ các ý này:
...
6. **Supervisor chủ yếu là attention trigger, không phải micromanager.** Nó phát hiện điểm cần chú ý, đặt câu hỏi mở dựa trên evidence và chỉ can thiệp sâu khi governance/recovery mandate yêu cầu.
7. **Heartbeat/event/detector là cơ chế đánh thức, không phải role.** Detection có thể dùng module/model rẻ; judgement vẫn thuộc Supervisor/Lead theo authority.
8. **Đối với quyết định nền tảng, đừng tin lane đầu tiên.** Dùng dual-lens/three-lens hoặc sealed council để tạo independent viewpoints trước khi Lead hội tụ.
9. **Test không được tự tạo contract.** Nếu representation/API/dependency chưa được quyết định, red test không được “mint” field/interface/adapter rồi ép production code chạy theo assumption đó.
10. **Context dài ít nguy hiểm hơn context rẽ nhánh.** Dependency lớn phát sinh có thể được tách sang Lead/lane khác rồi handback, thay vì làm Lead chính mất trajectory.
11. **Mục tiêu cuối là tiết kiệm Human attention.** Human giữ governance/boundary và chỉ xuất hiện ở quyết định có leverage cao, không theo dõi mọi prompt.

Mô hình gốc reference orchestration model hội tụ về:
...
Đây không phải hierarchy cứng `Supervisor > Lead`. reference orchestration model nói rõ:
...
| Nhãn | Ý nghĩa |
|---|---|
| **[DIRECT]** | reference orchestration model nói trực tiếp hoặc cung cấp prompt/config trực tiếp |
| **[SYNTHESIS]** | Mẫu thiết kế được tổng hợp từ nhiều message/context |
| **[PRACTICE]** | Heuristic/kinh nghiệm vận hành từ Episode 1; hữu ích để thiết kế workflow nhưng không phải kết luận đã được kiểm chứng độc lập |
...
invariant reference orchestration model đã nêu trực tiếp. Tài liệu không pha thêm cấu hình hoặc
...
Trong một số message ngày 01/08, reference orchestration model viết:
...
reference orchestration model sau đó nói “root này outdate rồi” (`1533108149706428578`). Vì vậy tài
...
## 2. Khi nào nên dùng SLP và khi nào không cần

SLP không phải ceremony bắt buộc cho mọi task.

**Không cần orchestration phức tạp** khi task nhỏ, có thể hoàn tất trong một vài lượt, ít dependency và không có decision architecture/ownership đáng kể. Một coding agent hoặc một Lead tự xử lý task tiny có thể đủ nếu protocol cho phép.

**Nên dùng SLP** khi một hoặc nhiều điều sau đúng:

- task/project kéo dài;
- Human không thể theo dõi liên tục;
- có nhiều quyết định architecture/lifecycle khó hoặc khó đảo ngược;
- domain mới làm Human chưa đủ tự tin để phản biện trực tiếp;
- cần nhiều phương án độc lập trước khi hội tụ;
- implementation có nhiều nhánh/dependency;
- quality cần review ở nhiều cấp;
- project lớn muốn giữ ownership/quality gate thống nhất ngay cả với những implementation task nhỏ.

Đối với Greenfield hoặc long-running work, quality phải được bảo vệ **trong quá trình chạy**, không phải đợi tới cuối mới review. Lead giữ decision/integration/acceptance, Peer giữ bounded implementation/judgment, Supervisor quan sát điểm rủi ro và kích hoạt attention khi cần.

### 2.1 Human attention là bottleneck hệ thống

Khi số project tăng, bottleneck chuyển từ “AI viết code nhanh đến đâu” sang “Human nên chú ý vào đâu và lúc nào”.

Mục tiêu của SLP là để Human tham gia **rời rạc nhưng có leverage cao**:

- không cần đọc từng prompt/session;
- nhận những decision/failure/risk đáng chú ý;
- giữ product/portfolio/irreversible/external-side-effect authority;
- để Lead tổ chức reasoning, multi-lane và reconciliation trong technical space mà Lead có context tốt hơn.

Điều này không có nghĩa Human luôn là node có knowledge kỹ thuật tốt nhất. Human vẫn là owner của governance và boundary, còn technical judgment có thể được giao cho Lead nếu Lead có evidence, codebase context và independent Peer input tốt hơn.

Một output hữu ích của Supervisor cho Human có thể là một digest ngắn thay vì transcript: decision lớn nào vừa xảy ra, agent nào đang blocked, test nào flaky, risk nào tăng, change nào khó đảo ngược và Human decision nào thực sự còn thiếu. Cách trình bày có thể là text hoặc voice tùy workflow; invariant là **nén attention**, không phải chuyển toàn bộ timeline cho Human.

### 2.2 Domain-first trước foundation work

Với foundation feature hoặc domain hoàn toàn mới, Human không cần tự thiết kế toàn bộ nhưng nên có framing tối thiểu trước khi giao orchestration:

1. đọc tài liệu/nguồn nền;
2. dùng advisory session hoặc Supervisor để dựng mental model;
3. xác định boundary, irreversible decisions và điều chưa biết;
4. sau đó mới để Lead mở dual-lane/three-lane design và đi tới implementation.

Mục tiêu là đủ hiểu để đặt boundary và nhận ra drift, không phải biến Human thành implementer.


## 3. Prerequisites: cần có gì trước khi dựng role
...
### 3.1 Một control plane duy nhất
...
### 3.2 Session độc lập, không fork bias khi cần độc lập
...
### 3.3 Workspace isolation cho writer
...
### 3.4 Provider và model phải được discover
...
### 3.5 Acceptance cần evidence, không dựa vào trạng thái
...
### 3.6 Human phải xác định các decision boundary
...
## 4. Ba lớp instruction: profile, protocol, task prompt
...
nào liên quan. Đó chính là lý do reference orchestration model không muốn bỏ Workspace Protocol vào
`AGENTS.md` mà mọi implementer đều đọc.

---

## 5. Role model chi tiết
...
Trong mô hình reference orchestration model, Human có thể trao đổi chủ yếu với Supervisor để không
...
### Supervisor như một attention trigger

Một failure của agent không nhất thiết đến từ thiếu năng lực; có thể agent chỉ chưa đặt attention đúng chỗ. Supervisor có giá trị khi **đánh thức đúng câu hỏi vào đúng thời điểm**.

Ví dụ nên hỏi:

- “Có contract/tài liệu nào đang bị bỏ qua không?”
- “Có anti-pattern nào có thể đang xảy ra không?”
- “Thay đổi implementation này có còn nằm trong write scope, hay đã chạm design/requirement?”
- “Có root mechanism nào giải thích chuỗi correction này không?”

Tránh framing thành verdict kiểu “bạn đang vi phạm anti-pattern X” khi evidence chưa đủ. Câu hỏi mở giảm nguy cơ agent cố tìm bằng chứng để làm vừa giả định của Supervisor.
...
detect signal
  → collect evidence
  → wake Supervisor
  → Supervisor asks/openly tests hypothesis
  → Lead/Peer re-reason
  → escalate only when authority/risk requires
```

Supervisor tốt không phải lớp control vi mô liên tục. Nó giữ context đủ sạch để nhận ra bias/drift nhưng không chen vào mọi bước.

### Detection, event, heartbeat và judgement

Phân biệt bốn khái niệm:

| Thành phần | Bản chất | Có authority/judgement không? |
|---|---|---|
| Event | tín hiệu trạng thái/semantic có ý nghĩa | Không |
| Detector/monitor rẻ | phát hiện hesitation, self-correction, semantic drift, stalled progress | Không hoặc rất hạn chế |
| Heartbeat | wake-up định kỳ, safety net khi thiếu event | Không |
| Supervisor | governance observer có judgement | Có, trong boundary của role |

Các event đáng đánh thức Supervisor có thể gồm:

- Lead vừa chốt decision lớn;
- Peer vật lộn với ambiguity;
- reasoning đổi hướng đáng kể;
- agent tự nhận ra lỗi;
- implementation decision bắt đầu đổi design/requirement;
- workflow mất momentum;
- repeated correction, tool failure hoặc scope movement;
- stable candidate/review finding xuất hiện.

Một detector/model rẻ có thể theo dõi event stream và chỉ phát event. Khi được wake, Supervisor mới quyết định có cần can thiệp. Đây là separation giữa **detection** và **judgement**.

Nếu chưa có event bridge, heartbeat thấp tần suất vẫn hữu ích như safety net. Ví dụ “khoảng 15 phút” trong nguồn chỉ là minh họa, không phải cadence bắt buộc. Không biến heartbeat thành polling loop hoặc hidden worker.

### Model tier cho Supervisor

Role name không quyết model tier. Có thể:

- dùng model rẻ/context lớn cho structured monitoring;
- escalate architecture audit, recovery hoặc judgement khó sang model mạnh hơn;
- luôn giữ cùng authority contract dù đổi model.
...
### Quản lý context: trajectory quan trọng hơn độ dài

Compact/handoff không phải vấn đề lớn nếu Lead vẫn đi trên một trajectory tương đối thẳng. Rủi ro lớn hơn là **context branching**: Lead đang giữ framing của một objective nhưng bất ngờ phải nuốt một dependency lớn, domain khác hoặc subsystem chưa tồn tại.

Ví dụ:

```text
Lead A đang làm authorization
  → phát hiện authentication foundation chưa tồn tại
  → không để Lead A bỏ trajectory và tự ôm cả authentication
  → tạo Lead/lane B cho dependency nếu boundary cho phép
  → B handback stable result/decision
  → A tiếp tục objective ban đầu
```

Tách nhánh giúp giảm context contamination và giữ coordination attention. Lead chính vẫn phải hiểu dependency contract, nhưng không cần tự gánh toàn bộ implementation của dependency.

Điều này cũng nhấn mạnh rằng Lead không chỉ là router. Lead giữ framing, đánh giá phương án, reconcile disagreement, quyết định khi nào mở lane mới và chốt project verdict.
...
reference orchestration model khuyến nghị một Peer profile với default Codex behavior cộng một vài
...
### Peer phải có quyền phản đối Lead

Nếu Lead chỉ hỏi “A hay B?” thì Peer đã bị khóa vào framing. Independent co-worker phải có quyền:

- chỉ ra premise/assumption sai;
- đề xuất phương án C ngoài options của Lead;
- trả `REOPEN_REQUEST`, `DEPENDENCY_REQUEST`, `BLOCKED`;
- challenge bằng evidence;
- không bị coi disagreement là disobedience.

Lead cũng phải biết **cách hỏi**: giữ preferred solution ở phía sau khi decision còn mơ hồ, đưa objective/constraints/evidence và đặt câu hỏi mở.

Nếu Peer A và B bất đồng, Lead có thể tổ chức một vòng phản biện chéo có giới hạn rồi reconcile. Mục tiêu là decision quality, không phải tranh luận vô hạn.

### Dual-lens, three-lens và blind design

Với decision có nhiều solution hợp lý hoặc có lock-in lớn, dùng nhiều lane độc lập:

```text
Problem + neutral constraints
  ├── Peer A → independent design
  ├── Peer B → independent design
  └── Peer C → independent design (nếu risk đáng)
        ↓
Lead compares propositions
        ↓
bounded challenge/rebuttal
        ↓
binding verdict
```

Để có divergence thật:

- không cho các lane biết trước preferred answer của Lead;
- khi cần, sealed reports trước khi xem output của lane khác;
- distinct mandate tốt hơn nhiều seat giống hệt nhau;
- không dùng vote popularity;
- cùng một model ở session khác nhau vẫn có thể tạo trajectory khác, nên multi-lane giảm “hên xui” của first answer.

Đây là Best-of-N ở mức workflow: tạo nhiều đường reasoning, sau đó dùng Lead để hội tụ có evidence.

### Design có thể bị implementation mở lại

Planning không thể dự đoán hết những gì lộ ra khi implement. Peer có thể phát hiện bandwidth, lifecycle, failure semantics hoặc accuracy trade-off mới.

Ví dụ một Peer thấy cần giảm precision, quantize `int16` → `int8`, giảm frequency hoặc đổi cadence. Nếu việc đó ảnh hưởng behavior, reconciliation, requirement hoặc architecture, Peer **không được coi nó là local implementation detail** chỉ vì mình có write access.

Phải phân loại:

```text
local detail trong authority
  → Peer quyết + verify

design/contract/lifecycle thay đổi
  → REOPEN_REQUEST về Lead
  → có thể mở Architect/multi-lane

owner/product/irreversible trade-off
  → Lead đưa evidence lên Human
```
...
#### 6.1.1 Core infrastructure phải generic; SLP là workflow opinionated

Một orchestration engine không nên hard-code SLP vào core. SLP phản ánh một workflow/opinion cụ thể; người khác có thể muốn topology khác, và chính workflow hiện tại cũng có thể tiến hóa.

Core nên cung cấp primitive generic:

- remote connection;
- multi-workspace management;
- provider/model connection;
- durable identity, lifecycle, parentage, timeline;
- event/schedule/heartbeat primitive;
- UI/UX quan sát và điều khiển;
- extension point cho workflow/plugin/profile.

SLP nên nằm ở profile/protocol/plugin layer và có thể bật/tắt/thay thế.
...
generic control plane
  ...
  + task assignment
  = concrete SLP workflow
```

Điểm mạnh của Paseo trong mô hình nguồn là nó đóng vai control plane thay vì ép người dùng theo SLP. Vì vậy continuous optimization nên patch profile/protocol/workflow layer, không fork core chỉ để sửa chiến thuật.


## 6.2 Provider tối thiểu theo reference orchestration model
...
## 6.5 Event-driven monitoring

reference orchestration model cảnh báo polling làm lãng phí context. Lead nên:
...
Heartbeat cần được hiểu là **wake primitive**, không phải một actor có ownership. Nó chỉ nên đánh thức Lead/Supervisor để inspect evidence mới. Nếu không có thay đổi đáng kể, không tự tạo work chỉ vì heartbeat chạy.

Primary path nên là event/notification; heartbeat chỉ là fallback/safety net khi event substrate chưa đủ.

---

## 7. `WORKSPACE_PROTOCOL.md`

## 7.1 Định nghĩa

`WORKSPACE_PROTOCOL.md` là policy chiến thuật điều phối riêng của một repository.

**[DIRECT]** reference orchestration model mô tả:
...
## 7.4 Protocol không nên chứa gì?
...
## 7.6 Ví dụ: protocol lỏng và chặt
...
mà reference orchestration model muốn giữ ở project layer.

---

## 8. Triết lý nền tảng

## 8.1 Independent co-worker, không phải function call
...
## 8.4 One writer, stable snapshot
...
## 8.5 Verification không đồng nghĩa acceptance
...
## 8.6 Sparse, event-driven supervision
...
## 8.7 Continuous optimization thuộc protocol
...
## 8.8 Skill topology theo attention
...
## 8.9 Continuous improvement từ failure thực tế

Không nên over-optimize policy ngay từ ngày đầu. Sau mỗi chu kỳ vận hành, review failure modes:

- Lead thường sai ở đâu?
- Peer thường sai ở đâu?
- Supervisor wake quá nhiều hay bỏ sót gì?
- pattern nào lặp lại trên nhiều repo?
- pattern nào generic đủ để vào role instruction/skill?
- pattern nào repo-specific nên vào `WORKSPACE_PROTOCOL.md`?
- có rule nào đang tạo ceremony mà không tăng evidence?

Quy trình Better SLP:

```text
failure/near miss
  → causal note
  → xác nhận có lặp lại không
  → distill generic invariant hoặc repo rule
  → version change
  → quan sát hậu quả
```

Không lưu mỗi verdict kiểu “Lead sai X”; phải lưu mechanism để tránh policy hóa một case ngẫu nhiên.

## 8.10 “Show, don’t tell” để điều khiển attention

Khi muốn agent tự kiểm tra lại reasoning, ưu tiên đưa evidence/case và câu hỏi mở hơn là đưa sẵn verdict. Điều này giúp agent tự tái dựng mechanism, giảm obedience bias và tạo lesson bền hơn.

Cùng nguyên tắc đó áp dụng cho Human/team training: case thực tế + causal explanation thường hữu ích hơn một rulebook ngày càng phình.
...
## 9.1 Sheep / authority-gradient compliance
...
## 9.2 Pre-solving / perfect-plan trap
...
## 9.3 Parachute optimization instead of brakes

Ví dụ trực tiếp của reference orchestration model: nhiều agent cùng tối ưu chiếc dù để xe đạp giảm
...
## 9.7 Self-benchmark / self-acceptance
...
## 9.9 Overengineering an edge case
...
## 9.18 Red test tự “mint” API/contract

**Dấu hiệu:** contract/representation chưa được quyết định nhưng test tự thêm field, interface, adapter, mock shape hoặc behavior mới để tạo red test.

Ví dụ: feature cộng điểm chưa quyết định data ownership; test tự giả định `User.point`, rồi implementer thêm `point` vào production model chỉ để test xanh.

**Cơ chế nguy hiểm:**

```text
unknown contract
  → test phải invent representation
  → red test được coi như contract
  → production code bị bẻ để pass
  → assumption ngẫu nhiên bị pin vào suite
  → session sau bảo vệ test cũ như source of truth
```

**Hậu quả:** overspecification, architecture debt, refactor khó, test suite trở thành technical debt thay vì safety net.

**Phản ứng:**

- nếu contract chưa rõ, **không viết unit test cấp contract**;
- làm rõ representation/API/dependency/ownership boundary trước;
- dùng `REOPEN_REQUEST`/Architect/Lead decision nếu cần;
- test phải kiểm chứng contract, không được tự tạo contract;
- hỏi: “test này sẽ fail dưới mechanism sai nào?” và “behavior đang test đã được quyết định ở đâu?”.

Nếu AI viết phần lớn behavior tests hoặc coverage cao, anti-pattern này càng cần được encode thành instruction/review rule vì một assumption sai có thể nhân rộng thành hàng loạt test và technical debt.

## 9.19 Context-branch contamination

**Dấu hiệu:** Lead đang giữ một trajectory nhưng liên tục nuốt dependency mới, domain mới và implementation branch không thuộc objective ban đầu.

**Cơ chế:** coordination context bị thay bằng local implementation context; framing ban đầu mờ dần.

**Phản ứng:** tách dependency thành lane/Lead riêng khi đủ lớn, định nghĩa handback contract, giữ Lead chính làm integration/decision owner.

## 9.20 Verdict-first supervision

**Dấu hiệu:** Supervisor nói “đây là anti-pattern X” trước khi evidence đủ, khiến Lead/Peer tìm lỗi để xác nhận framing.

**Phản ứng:** observation → evidence → open question → hypothesis test. Supervisor chỉ ra correction order khi authority và evidence thực sự yêu cầu.


---

## 10. Topology theo độ khó
...
### Multi-lens / blind design cho decision nền tảng

```text
Lead defines problem + constraints, hides preferred answer
  ├── Architect/Peer A (sealed)
  ├── Architect/Peer B (sealed)
  └── optional Peer C (sealed)
        ↓
Lead extracts material propositions
        ↓
bounded cross-challenge
        ↓
Lead binding verdict
        ↓
Human only if decision crosses owner boundary
```
...
```text
Lead/Peer activity
  → event/detector OR low-frequency heartbeat
  → Supervisor reads only material delta/evidence
  → no issue: no intervention
  → suspected issue: open evidence-backed question
  → Lead/Peer re-reason
  → Human/recovery escalation only when boundary requires
```
...
- [ ] Heartbeat không bị dùng như polling worker; event/notification là primary khi có thể.
- [ ] Supervisor hỏi mở từ evidence, không verdict-first.
- [ ] Decision nền tảng có independent lens nếu risk/lock-in yêu cầu.
- [ ] Peer không tự mint API/contract trong test khi foundation chưa ổn định.
- [ ] Dependency branch lớn được tách/handoff thay vì làm Lead mất trajectory.

### 11.3 Trước acceptance
...
### 11.4 Checklist riêng cho Supervisor

- [ ] Tôi đang quan sát workflow hay đang biến mình thành Lead thứ hai?
- [ ] Tín hiệu này có evidence hay chỉ là cảm giác?
- [ ] Câu hỏi có trung lập hay đã chứa verdict?
- [ ] Có cần wake bây giờ không, hay event chưa decision-changing?
- [ ] Issue nằm trong Lead authority, Human authority hay chỉ là local Peer detail?
- [ ] Nếu cần recovery, mandate có explicit không?
- [ ] Notebook ghi causal mechanism, không chỉ ghi lỗi/v verdict?
- [ ] Pattern đã đủ lặp để patch protocol/profile chưa?

### 11.5 Checklist riêng cho Lead

- [ ] Tôi đã reconstruct objective nhưng tránh pre-solve?
- [ ] Brief có để Peer đề xuất solution ngoài framing của tôi?
- [ ] Mỗi moving scope có đúng một owner?
- [ ] Decision khó có cần dual-/three-lens hoặc sealed review?
- [ ] Peer disagreement đã được reconcile bằng evidence?
- [ ] Dependency mới có làm context rẽ nhánh quá xa không?
- [ ] Candidate đã stable trước review?
- [ ] Test đang kiểm contract thật hay tự mint contract?
- [ ] Tôi đang giữ project acceptance, không tự biến “done/tests pass” thành verdict?

### 11.6 Checklist riêng cho Peer

- [ ] Tôi hiểu objective, scope, exclusions, authority và verification?
- [ ] Tôi có đang implement preferred solution của Lead mà chưa kiểm foundation?
- [ ] Nếu premise/API/ownership sai, tôi đã `REOPEN_REQUEST` thay vì patch vòng?
- [ ] Tôi có tự mở rộng scope hoặc external side effect không?
- [ ] Test của tôi có invent API/field/adapter chưa được quyết định không?
- [ ] Tôi đã proof write của mình bằng exact commands/results?
- [ ] Tôi đang báo exact artifact/risk/assumption, không chỉ “done”?

---

## 12. Human practice, learning và quality ngoài orchestration core

### 12.1 Từ prompt skill sang AI management

Khi model mạnh hơn, leverage chuyển dần từ “prompt câu nào hay” sang:

- decomposition và delegation;
- boundary/authority design;
- tạo independent viewpoints;
- biết khi nào Human cần vào loop;
- đánh giá evidence và final decision;
- distill failure thành protocol/skill.

Nhưng AI management không thay thế domain expertise. Human vẫn cần học đủ sâu để thiết kế governance, nhận ra weak evidence và hiểu trade-off.

### 12.2 Frontend AI slop: quality gate cần thẩm mỹ, không chỉ prompt

Một bài học ngoài orchestration core nhưng liên quan trực tiếp đến review quality: muốn phát hiện frontend “AI slop”, reviewer phải xây visual taste bằng reference tốt.

Khi xem sản phẩm tốt, phân tích:
...
- spacing;
- hierarchy;
- yếu tố tạo cảm giác chuyên nghiệp;
- reusable patterns.

Kho reference/bookmark giúp Human hình thành tiêu chuẩn để review AI output. Orchestration không thể bù cho một acceptance standard mơ hồ.

### 12.3 Ví dụ game/asset chỉ là minh họa, không phải core rule

Nguồn có các ví dụ về multiplayer synchronization, MMO/networking và pixel animation. Chúng hữu ích để minh họa rằng:

- nhiều state có thể cần replication strategy khác nhau;
- implementation có thể lộ bandwidth/precision trade-off làm design phải reopen;
- AI-assisted asset generation/segmentation là domain technique, không phải orchestration invariant.

Không đưa các kỹ thuật game/asset này vào global role profile trừ khi repository thật sự thuộc domain đó.

---

## 13. Kết luận

Giá trị lớn nhất trong mô hình reference orchestration model không nằm ở tên `Root`, `Lead`,
...
## 14. Distilled operating contract — agent có thể dùng như quick reference

```text
HUMAN
  owns product/portfolio/irreversible/external-side-effect boundaries
  defines acceptable evidence and authority

SUPERVISOR
  observes workflow/reasoning
  is an attention trigger, not default implementer
  treats anti-patterns as hypotheses
  uses events/heartbeat only to wake judgement
  records causal context and escalates when boundary requires

LEAD
  owns project framing, topology, ownership, dependencies, integration and acceptance
  asks neutral/open questions
  opens independent lanes for hard decisions
  keeps one writer per moving scope
  reviews only stable candidates
  separates lifecycle status from acceptance
  splits large dependency branches when trajectory would be contaminated

PEER
  owns one bounded outcome
  forms independent judgement
  may challenge/reopen/block with evidence
  does not orchestrate
  does not expand authority
  verifies own write but does not self-accept hard changes
  never lets tests invent undecided contracts

CONTROL PLANE / PASEO
  owns lifecycle/workspace/identity/parentage/timeline primitives
  does not hard-code repo strategy
  SLP lives in profiles/protocol/assignment, not core

WORKSPACE_PROTOCOL
  holds repo-specific topology, risk, review, routing and proof rules
  Lead reads it; Peer receives only task-relevant constraints

MONITORING
  events first
  heartbeat as low-frequency safety net
  detector may be cheap
  judgement remains with Supervisor/Lead

PROOF
  exact artifact
  stable candidate identity
  exact verification commands/results
  independent falsification when risk requires
  authority-correct acceptance
```

### Không được quên

1. Đừng biến Peer thành function call.
2. Đừng pre-solve rồi gọi đó là delegation.
3. Đừng dùng nhiều agent như vote popularity.
4. Đừng review moving target.
5. Đừng coi `finished`, `idle` hay test pass là acceptance.
6. Đừng để heartbeat thành polling loop.
7. Đừng để Supervisor micromanage.
8. Đừng để red test tự mint API/contract.
9. Đừng để Lead nuốt mọi dependency branch.
10. Đừng hard-code SLP vào orchestration core.
11. Đừng tối ưu policy trước khi có failure evidence.
12. Mục tiêu là bảo vệ independent judgment, evidence chain và Human attention.
