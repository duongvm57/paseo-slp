...
Status: initial Paseo 0.8.0 audit extended with source checks and independently reproduced offline spikes. Option A transaction/rebind and B2 per-family lifecycle analysis remain incomplete; no live plugin acceptance. The follow-up findings and recovery checkpoint below qualify or supersede earlier conclusions.
...
At the audited baseline, `AGENTS.md` named `docs/reference/agent-orchestration-complete-operating-guide.md`, which remains absent from the repository. At current HEAD (`ee8c2a7`) `AGENTS.md` references only [the candidate contract](contract.md), and the guide exists solely as an untracked local artifact at `.local-checks/reference/agent-orchestration-complete-operating-guide.md`. This audit does not invent a replacement. Existing SLP behavior is taken from the contract and the implementation it maps.
...
| Inject Claude role policy | `systemPrompt`; Claude composes it into the Agent SDK `systemPrompt:{preset:"claude_code",append}`, delivered as the initialize `systemPrompt.append` field the current wrapper already sets | PROVEN for source path | Cleanest native mapping of the four families; imported native sessions and disabled-plugin launches still stay unroled. Current wrapper remains the low-risk route. |
...
| Change parent, labels, or workspace in agent hook | Fields are absent; `cwd` cannot change | UNSUPPORTED | Keep routing/parentage in the existing creation flow and evidence. |
...
**Option A decision:** **CONDITIONAL, not yet GO.** It remains the lowest-change candidate, but the config-store follow-up adds explicit concurrency, field-restoration and recovery constraints that must be resolved alongside the live gates before production commitment.
...
Claude-specific additions: `--version`/`auth status` admin probes must pass through the bootstrap to the real binary (provider env grants are absent on no-env spawns); catalog fetch spawns no session; draft-command listing spawns a real stream-json session without grant env and must complete its initialize round-trip without forwarding any `type:"user"` frame.
...
Run the SLP E2E manifest required by the changed transport/lifecycle contract, including all twelve `resume-{role}-{provider}` scenarios (48 scenarios total at current HEAD `ee8c2a7`; onboarding now covers `{project,global}×{codex,pi,claude}`). Existing U2 evidence explicitly correlates provider command, installed bytes, actual initial/resume input, settings, parent create arguments, and triggered reference reads. Local typechecks and transport unit tests are not E2E acceptance.
...
- Three independent runs (`run-3tju9b` by the author, `run-AFe2wV` and `run-2DnucI` by the coordinator) coexist under `.local-checks/plugin-candidate-roundtrip/` with identical results; earlier fixed-name fixture directories from the first spike iteration remain untouched.
...
| `launchPlan`, `handoffPlan` | Preserve complete bindings, `inventoryFile`, `assignmentFile`, settlement and creation arguments. They do not create agents; handoff calls `snapshot`, which runs Git subprocesses, so it is not pure filesystem-only code. `src/launch.mjs:53–88,117–156`, `src/package.mjs:55–89`. |
...
| `initWorkspace` | Preserve preview/apply, explicit `--routing-from`, daemon-local repository paths and no-overwrite behavior. `src/paseo-install.mjs:172–205`. |
| Onboarding and host activation | Stable skill files remain accessible; no automatic global skill installation or workspace writes. Current host install/upgrade does not manage routing catalogs; migration is explicit. `docs/contract.md:63–64`. |
...
- **The SDK config path is strictly weaker than the current installer on concurrency.** The file-path installer has byte-level CAS (`src/host-config.mjs:26–28`); `config.patch` has no revision/expected-base field (`messages.d.ts:154–213`), so the plugin path loses a primitive the current package already relies on.
...
- **Correct discriminator: a dedicated `SLP_SESSION_OPEN_GRANT`.** Provider runtime `env` sets `SLP_SESSION_OPEN_GRANT:""` for every spawn (schema `z.record(z.string(),z.string())` accepts empty; provider env reaches all spawn paths including probes); the `session_open` overlay writes the real grant and wins (`provider-launch-config.js:127–143`). The sentinel simultaneously neutralizes stale inherited grants. Bootstrap state machine: unset/empty → discovery allowlist; nonempty+valid → role; nonempty+malformed/tampered → hard fail. Instruction methods are denied in discovery mode, so a grantless open (disabled/crash/reload gap) fails closed at the first instruction frame without breaking discovery — no need to classify spawn purpose up front.
- **No-env spawn surfaces are now enumerated per family.** Codex has the widest set (catalog, importable list, both draft fallbacks, archive/restore sync where restore hard-fails if `thread/archive`/`unarchive` are denied, and the `argv[0] --version` probe which mis-parses Node as codex ≥0.128 → goals/auto-review gates must be handled by bootstrap self-probing the real binary). Pi: real `--mode rpc` catalog + draft-commands fallback (`listFeatures` returns `[]` client-side; importable list is filesystem-only). ACP: catalog probe performs `session/new`; `listFeatures` also probes `session/new`; `session/load` must be allowed for import preview while `session/prompt` is denied.
- **Wire-verified allowlists** for the three original families are recorded in `.local-checks/plugin-followup-b2/allowlists.json`; the fixture proves the state-machine shape, not allowlist completeness — real method matrices still need live enumeration (S4).
...
- `agent.create.config.systemPrompt` composes into the Agent SDK `systemPrompt:{preset:"claude_code",append}` (`claude/agent.js:2588–2637`) and arrives as the initialize `systemPrompt.append` field — the same field `claudeRolePrompt` already sets (`src/role-transport.mjs:35–49`). This is the cleanest native systemPrompt mapping of the four families for a future B0 simplification; import of native `~/.claude/projects` sessions stays unroled because metadata lacks `systemPrompt`.
...
- B2 instruction boundary for Claude is a single frame type: deny `type:"user"` frames in discovery mode; allow `control_request`/`control_response` initialize traffic. The bootstrap must self-resolve the real binary for `--version`/`auth status` probes (`SLP_CLAUDE_BIN` is not available in no-env spawns). Claude is the lowest-residual B2 family, but consistent with the conservative policy it remains CONDITIONAL until its live gates run — no family gets an unconditional GO without live evidence.
...
- **Byte-exact restoration → semantic restoration.** `injectIntoAgents:false` is semantically equal to absent (`config.js:338`); patches cannot express key deletion. Receipts record pre-patch presence; deactivate restores semantics.
...
- collisions adopt only byte-identical entries under the **persisted** schema — `get()` is normalized and strips unknown keys (`daemon-config-store.js:182–203`);
...
- Two config-write strategies: **A1** `config.patch` (default, constraints above); **A2** file-write through `src/host-config.mjs` (real byte-CAS, `host-config.mjs:24–35`) + `paseo reload` — preserves today's exact-restoration contract but needs a verified daemon home and CLI reachability on the daemon host. `persistConfig` re-reads the file on every patch, so the strategies interleave consistently.
...
| B0 fails only on Codex/Pi | Slightly wrong — Claude also accepts `systemPrompt` (native append); only Devin/ACP is the hard blocker |
...
## Research status checkpoint (current HEAD ee8c2a7→d30dacb, candidate unchanged)
...
- B2: corrected grant design verified (`.local-checks/plugin-followup-b2-findings.md`, 33/33 local tests): dedicated `SLP_SESSION_OPEN_GRANT` sentinel+overlay replaces the unsafe `PASEO_AGENT_ID` discriminator; per-family no-env surfaces and wire-verified allowlists enumerated; `node -e` ruled out → executable shim. Verdict: **CONDITIONAL/LIVE-SPIKE, experimental only** for all four families (Claude lowest residual).
